...
//   const element = miCamino.cimas[i];
...
for (let i = 0; i < miCamino.material.length; i++) {
  const element = miCamino.material[i];
  for (let j = 0; j < element.imagenes.length; j++){
    for (const property in element.imagenes[j]){
      console.log(element.imagenes[j][property].url)
    };
  }
...
for (let i = 0; i < miCamino.cimas.length; i++) {
     const element = miCamino.cimas[i]; 
     cantidadCimas += 1;
     totalAltitud = totalAltitud + (element.altitud * 1000); //multiplico *1000 ya que en la altitud hay puntos como simbolo de miles e interpreta(8,339 en vez de 8339)
    }
  
  altitudMedia = totalAltitud / cantidadCimas;

    console.log(`La altitud media de las cimas es de: ${Math.round(altitudMedia)} metros`);